...
| V10 | Widget bundles Bootstrap 3.4.1 and jQuery 3.6.0 by default; suppress with `includeBootstrap=false` / `includeJQuery=false` | Guide p.15 | `[DONE 2026-08-22]` Both suppressed in `MapWidgetEmbed.tsx`. We use neither, and Bootstrap's reset collides with Tailwind |
