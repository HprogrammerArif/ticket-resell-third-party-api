...
| V9 | Mobile layout (<992px) expects the **full screen** — site header and footer must be hidden via CSS or layout calculations break | Guide p.13, p.24 | Not yet implemented. Real bug risk on mobile event pages |
| V10 | Widget bundles Bootstrap 3.4.1 and jQuery 3.6.0 by default; suppress with `includeBootstrap=false` / `includeJQuery=false` | Guide p.15 | We use neither — should be suppressed to avoid conflicts |
