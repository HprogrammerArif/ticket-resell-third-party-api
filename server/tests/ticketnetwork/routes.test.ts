...
describe('Catalog routes', () => {
  ...
  describe('GET /api/catalog/categories', () => {
    ...
    it('forwards pageSize and pageNumber as numbers', async () => {
      ...
      await request(app).get('/api/catalog/categories?pageSize=5&pageNumber=2');
  ...
  describe('GET /api/catalog/categories/:path (nested)', () => {
    it('passes the full nested path to getCategoryByPath', async () => {
      ...
      const res = await request(app).get('/api/catalog/categories/sports/hockey');
      expect(res.status).toBe(200);
      expect(catalog.getCategoryByPath).toHaveBeenCalledWith('sports/hockey', expect.any(Object));
    });
  });

  describe('GET /api/catalog/events/search', () => {
    it('calls searchEvents (not getEventById)', async () => {
      ...
      const res = await request(app).get('/api/catalog/events/search?keyword=hockey');
      expect(res.status).toBe(200);
  ...
  describe('GET /api/catalog/search/suggest', () => {
    it('passes q to globalSuggest', async () => {
      ...
      const res = await request(app).get('/api/catalog/search/suggest?q=leaf');
      expect(res.status).toBe(200);
      expect(catalog.globalSuggest).toHaveBeenCalledWith('leaf', expect.any(Object));
    });

    it('returns 422 when q is missing', async () => {
      const res = await request(app).get('/api/catalog/search/suggest');
      expect(res.status).toBe(422);
      expect(res.body.error.code).toBe('VALIDATION_ERROR');
    });
  });
